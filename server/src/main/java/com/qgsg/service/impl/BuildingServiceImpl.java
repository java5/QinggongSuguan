...
public class BuildingServiceImpl implements BuildingService {
    ...
    public void saveDormitoryBuilding(BuildingDTO buildingDTO) {
        Building building1 = buildingMapper.select(buildingDTO.getBuildingNumber());
        if(building1!=null){
            throw new InsufficientCapacityException("该宿舍楼已存在！");
        }
        if(!(buildingDTO.getBuildingDetail().equals("男宿舍")||buildingDTO.getBuildingDetail().equals("女宿舍"))){
            throw new InsufficientCapacityException("只能备注为'男宿舍'或'女宿舍'");
        }else {
